...
/**
 * Uses GMM-based weights, with option of selecting K nearest target points to each model point
 */
public class GMMMeshCorrespondence extends MeshCorrespondenceComputer {
   ...
   // properties
   static PropertyList myProps = new PropertyList (GMMMeshCorrespondence.class, MeshCorrespondenceComputer.class);
   static {
   ...
   private static class VertexEntry {
      ProbEntry[] P = new ProbEntry[0]; // limit
   ...
   private void updateAllDistances() {
      ...
      for (Vertex3d vtx : model.getVertices ()) {
         ...
         // maybe create P row
         if (ventry.P.length != target.numVertices ()) {
            ventry.P = new ProbEntry[target.numVertices ()];
            for (int i=0; i<ventry.P.length; ++i) {
               ventry.P[i] = new ProbEntry();
            }
         }
         ...
         for (Vertex3d tvtx : target.getVertices ()) {
            ProbEntry pentry = ventry.P[nidx];
   ...
   /**
    * Number of nearest target points to consider.  A zero indicates to use all targets.
    * 
   ...
   /**
    * Sets the number of nearest target points to consider.  This can help reduce the complexity
    * from M*N (where M is number of model points, N is number of target points) to M*K*log(N).
    * Set to 0 to consider all targets
    * 
   ...
   private void updateKDistances(int k) {
      ...
      for (Vertex3d vtx : model.getVertices ()) {
         ...
         // find k nearest target vertices
         TreeSet<ObjectDistanceEntry<Feature>> nearest;
         if (perp) {
            Vector3d nrm = new Vector3d();
            vtx.computeWorldNormal (nrm);
            nearest = nearestKVertexNormalToPoint (bvq, targetTree, k, vtx.getWorldPoint (), nrm);
         } else {
            nearest = bvq.nearestKFeaturesToPoint (targetTree, k, vtx.getWorldPoint ());
         }
          
         // maybe create sparse P row
         if (ventry.P.length != nearest.size ()) {
            ventry.P = new ProbEntry[nearest.size ()];
         ...
         for (ObjectDistanceEntry<Feature> ode : nearest) {
            ...
            pentry.n = ((Vertex3d)ode.getObject ()).getIndex ();
   ...
   public MatrixNd getProbabilityMatrix() {
      ...
      for (int m=0; m<vinfo.length; ++m) {
         for (int i=0; i<vinfo[m].P.length; ++i) {
            Pmat.add (m, vinfo[m].P[i].n, vinfo[m].P[i].p);
         }
      }
      ...
      if (vinfo != null) {
         ...
         for (VertexEntry ventry : vinfo) {
            ...
            if (ventry.psum > 0) {
               ...
               // other weighted average of all points
               double w = 1.0/ventry.P.length;
               for (ProbEntry pmn : ventry.P) {
                  ventry.x.scaledAdd (w, target.getVertex (pmn.n).getWorldPoint ());
                  target.getVertex (pmn.n).computeWorldNormal (n);
                  ventry.n.scaledAdd (w, n);
               }
               ventry.n.normalize ();
            }
         }
   ...
   @Override
   public GMMMeshCorrespondence clone () {
      GMMMeshCorrespondence gmm = (GMMMeshCorrespondence)super.clone ();
      gmm.setAutoVariance (isAutoVariance ());
      gmm.setNearestK (getNearestK ());
      gmm.setVariance (getVariance ());
      gmm.setNoiseFraction (getNoiseFraction ());
      gmm.setPerpendicular (getPerpendicular());
      return gmm;
   }
