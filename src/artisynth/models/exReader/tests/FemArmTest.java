package artisynth.models.exReader.tests;
...
import artisynth.models.exReader.CmissFileGrabber;
import artisynth.models.exReader.ExMeshGenerator;
import artisynth.models.exReader.ExParser;
import artisynth.models.exReader.ExRegion;
import artisynth.models.toolbox.femtool.FemTools;

public class FemArmTest extends RootModel {

   private static final String localPath = ArtisynthPath.getSrcRelativePath(
      FemArmTest.class, "data/cmiss/");
   CmissFileGrabber cmiss = new CmissFileGrabber(localPath);
   ...
   public FemArmTest (String name) throws Exception {
   ...
   public void addBones() {
      try {
         InputStream bonesStream = cmiss.getInputStream(armBonesFilename); 
         addMeshesFromStream(bonesStream, armBones, boneRenderProps);
      } catch (IOException e) {
         e.printStackTrace();
      }
   ...
   public void addMuscles() {
      ...
      try {
         InputStream musclesStream = cmiss.getInputStream(armMusclesFilename);
         muscleList = addFemFromStream(musclesStream, muscleRenderProps);
      } catch (IOException e) {
         e.printStackTrace();
         return;
      }
   ...
   public void addMeshesFromStream(InputStream stream, ArrayList<RigidBody> rbList,
      RenderProps props) throws IOException {

      ReaderTokenizer rtok = null;
      rtok = new ReaderTokenizer(new InputStreamReader(stream));
      ...
      while (rtok.nextToken() != ReaderTokenizer.TT_EOF) {
         if (rtok.ttype == ReaderTokenizer.TT_WORD) {
            ...
            InputStreamReader nodeReader = new InputStreamReader(
               cmiss.getInputStream(armPath + name + ".exnode"));
            InputStreamReader elemReader = new InputStreamReader(
               cmiss.getInputStream(armPath + name + ".exelem"));

            myParser.parseExNode(new ReaderTokenizer(nodeReader));
            myParser.parseExElem(new ReaderTokenizer(elemReader));
   ...
   public ArrayList<FemModel3d> addFemFromStream(InputStream femList,
      RenderProps props) throws IOException {

      ArrayList<FemModel3d> myFEMs = new ArrayList<FemModel3d>();

      ReaderTokenizer rtok = null;
      rtok = new ReaderTokenizer(new InputStreamReader(femList));

      while (rtok.nextToken() != ReaderTokenizer.TT_EOF) {
         if (rtok.ttype == ReaderTokenizer.TT_WORD) {
            
            InputStreamReader nodeReader = new InputStreamReader(
               cmiss.getInputStream(armPath + rtok.sval + ".exnode"));
            InputStreamReader elemReader = new InputStreamReader(
               cmiss.getInputStream(armPath + rtok.sval + ".exelem"));
