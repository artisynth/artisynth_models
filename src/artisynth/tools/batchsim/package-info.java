/**
 * Package implementing the <i>Batch Simulation Framework</i>.
 * <br>
 * <br>
 * <h1>1. INTRODUCTION</h1>
 * The Batch Simulation Framework, also known as BatchSim, is a framework that allows
 * a large number of simulations to be automatically performed using any arbitrary
 ...
 * {@link maspack.properties.Property Property} mechanism. See the
 * <a href="http://artisynth.magic.ubc.ca/pmwiki.php?n=Documentation.MaspackRefManual">
 ...
 * property specification, the given {@code Property} to each value in its value
 * set in turn. Thus, the user-specified {@code Property}({@code ies}) serve as
 * variable input(s) into the model, and a simulation is performed for each such
 * input, allowing some variable-dependent output(s) of the model to be recorded.
 * <p>
 ...
 * parametric probability distribution</a>, and BatchSim will perform simulations,
 * where, for each simulation, the given {@code Property} is set to a random
 * value sampled from its probability distribution. In this case, a
 * {@code Property} is no longer just a variable, but rather a true
 ...
 * part, the input value of any {@code Property} in any subsequent simulation.
 * A direct consequence of this assumption is that each simulation is entirely
 * independent of each other simulation. As such, performing multiple simulations
 * becomes an <a href="https://en.wikipedia.org/wiki/Embarrassingly_parallel">
 ...
 * manager informs the worker that there are no more simulation tasks to
 * complete. The workers complete their tasks independently and in parallel,
 * allowing the entire problem (running a set of simulations) to be solved in
 * less time.
 * <p>
 ...
 * A simulation task consists of a list of property-value pairs. The property
 * is either the name of a {@code Property} or else a property path (the path
 * from the {@code RootModel} to one of its {@code ModelComponent}s, followed
 * by `{@code :}', followed by the name of a property of that component). For
 ...
 * specifications and <b>probabilistic</b> specifications.
 * <br>
 * <h2>2.1. Combinatorial Property Specifications</h2>
 * Here, a particular {@code Property} is associated with a set of values, and
 * each value is assigned to the {@code Property} in turn. Other
 * {@code Properties} are associated with different value sets in their own
 * combinatorial specification. The manager iterates through the values in the
 * specifications so as to perform an exhaustive search of the input space:
 * each possible combination of property-value pairs for all combinatorial
 * specifications combined is examined. Each simulation tasks consists of
 * exactly one such combination. More formally, say we have {@code N} such
 * combinatorial specifications. Then, the general exhaustive-search/combinatorial
 ...
 * </pre>
 * 
 * Mathematically speaking, one simulation task is performed for each element
 * in the set that results from taking the
 * <a href="https://en.wikipedia.org/wiki/Cartesian_product"> Cartesian product</a>
 * of all given value sets.
 * 
 * <h2>2.2. Probabilistic Property Specifications</h2>
 * Here, a particular <u>numeric</u> {@code Property} is associated with a
 * vector of parametric probability distributions, with the size of the
 * probability distribution vector equal to the size of the {@code Property}
 * vector. For a scalar numeric {@code Property}, the probability distribution
 * vector should contain a single entry. Other {@code Properties} are
 * associated with different probabilistic distribution vectors in their own
 * probabilistic specification. If <b>only</b> probabilistic specifications are
 * given, the manager creates simulation tasks by applying the Monte Carlo
 * method. That is, for each probabilistic specification, all the distributions
 * listed in its associated probability distribution vector are sampled, and a
 * random-value vector is formed from the sampled values. These sampled vectors
 * are then assembled into one simulation task (thus forming one Monte Carlo
 * simulation task), and the process is repeated {@code M} times, where
 * {@code M} is specified by the user. Formally, the algorithm is as follows:
 * 
 * <pre>
 * for i in 1 to M
 *    vectors = new List()
 ...
 * specification are given, the combinatorial algorithm described above is
 * used, but augmented with a check for the specification's type. If the
 * specification is probabilistic, it is treated as if its distribution vector
 * was a single combinatorial value (whose value just happens to be different
 * with each combination). The end result is a hybrid algorithm between
 ...
 * </pre>
 * 
 * <h2>2.3. Property Specifications: Format</h2>
 * A number of combinatorial and/or probabilistic property specifications are
 * typically listed in an input file, which is then provided to the manager.
 * The input file format for both property specifications types is as follows:
 * <ul>
 * <li>Each <i>combinatorial</i> specification must adhere to the following
 * general format:<br>
 * <code>&nbsp;&nbsp;&nbsp;&lt;property_path&gt; = &lt;value_set&gt;</code><br>
 * where {@code <property_path>} must be a valid {@code ArtiSynth} property
 * path except for an optional component identifier set extension provided by
 * BatchSim that allows multiple components to be grouped into a single
 * specification (see below); and where {@code <value_set>} must be of the
 * form: <br>
 * <code>&nbsp;&nbsp;&nbsp;{<br>
 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;delim&gt;&lt;val1&gt;&lt;delim&gt;<br>
 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;delim&gt;&lt;val2&gt;&lt;delim&gt;<br>
 ...
 * <ol>
 * <li>
 * <code>"models/0/component/1:someProp.someSubProp" = {%1% %2% %3%}</code>
 * </li>
 ...
 * path except for an optional component identifier set extension provided by
 * BatchSim that allows multiple components to be grouped into a single
 * specification (see below); and where {@code <distribution_vector>} must be
 * of the form:<br>
 * <code>&nbsp;&nbsp;&nbsp;[<br>
 ...
 * in the same quantity, as specified by the corresponding {@code Distribution},
 * and each value must be a valid number for that particular parameter of that
 ...
 * the number of {@code Distributions} listed should match the vector's size.</li>
 ...
 * distribution according to which each entry of <code><b><i>X</i></b></code>
 * is distributed (using the distribution vector that follows {@code ~}).</li>
 ...
 * </li>
 * </ul>
 * <h2>2.4. Property Specifications: Syntax</h2>
 * Some notes on the input file syntax (in terms of tokens) of property
 * specifications follow. Unless otherwise indicated, syntax rules apply to
 * both types of property specifications.
 * <ul>
 * <li>The `{@code #}' token begins a comment. All characters between it and
 * the next end-of-line character are ignored.</li>
 ...
 * <li>The {@code <property_path>} token must be followed by a `{@code =}'
 * token (for combinatorial specifications), or by a `{@code ~}' token (for
 ...
 * specifications).</li>
 * <li>The list of {@code <valX>} tokens in the value set of every
 * combinatorial specification must all lie within the pair of curly braces.
 * The individual values must be enclosed in a pair of delimiter characters
 * (which is the `{@code %}' token by default). The following examples are all
 * syntactically valid and semantically equivalent <i>combinatorial</i>
 ...
 * `<code>{}</code>'. In this case, but <u>only</u> in this case, the
 * associated {@code Property} will be assigned the value <b>{@code null}</b>
 * in all simulation tasks.</li>
 ...
 * exection to this rule is that non-end-of-line whitespace appearing within
 * a component identifier set of {@code <property_path>} token is ignored (see
 * below).</li>
 ...
 * used as a shorthand to group multiple subcomponents of a particular
 * component into a single specification. This is useful if some or all
 * subcomponents would otherwise be specified as having the same value set (for
 * combinatorial specifications) or the same distribution vector (for
 * probabilistic specifications).</li>
 ...
 * <li>Note that the first integer specified in a range must be less than or equal
 * to the second integer in the range, but separate ranges need not be listed in
 * increasing order.</li>
 ...
 * identifier listed in the set. For example, the {@code <property_path>} token<br>
 * <code>
 ...
 * Prerequisites for using BatchSim:
 * <ul>
 ...
 * subclass. This is required only if a custom worker subclass is being used,
 * as otherwise the {@code batchDriver.py} script in this package can be used
 * to drive the {@code SimpleTimedBatchWorker}.</li>
 ...
 * <li> (Optional) A copy of the {@code jdistlib} jar available from <a
 * href="http://jdistlib.sourceforge.net/">http://jdistlib.sourceforge.net/</a>.
 * This is only required if at least one property specification is
 * probabilistic. For purely combinatorial specifications, {@code jdistlib}
 * is not required. Ensure that the downloaded jar file is on the build path or
 * the class path prior to compilation. For example, the jar file can be placed
 * in {@code $ARTISYNTH_HOME/lib} (for Linux/Mac OSX) or
 ...
 * </ul>
 * BatchSim can be run both from within {@code Eclipse} and from the command-line.
 * Although running from the command-line may seem daunting at first, doing so
 * is actually easier than running from within {@code Eclipse}, because BatchSim
 * can run from the directory in which the input file and optional
 * {@code Jython} script already reside, and each manager and worker gets its
 * own output console.
 * <p>
 ...
 * "props.txt", which is the default filename that the manager looks for. If
 ...
 * {@code ArtiSynth}. The only benefit to running the manager within an
 * instance of {@code ArtiSynth} is that only then can the manager verify that
 * the property specifications listed in the input file represent valid
 ...
 * validate the property specifications, but unlike the manager, this
 * validation is done only as quickly as the worker receives property-value
 * pairs from the manager in the form of simulation tasks.</li>
 * <li>Decide how many workers to run. Then, open that many additional
 * terminal windows (one for each worker), and {@code cd} to {@code batch} in
 * each one.</li>
 ...
 * <li>If no errors have been made, BatchSim should now be performing simulations
 * as per the property specifications in the input file. In addition, the
 * simulations will be performed in parallel if more than one worker was
 * started.</li>
 * 
 * Running BatchSim from within {@code Eclipse} is done is a similar fashion, except
 * that in {@code Eclipse} the working directory is the project root directory
 * (i.e. {@code artisynth_core}, {@code artisynth_models}, or
 ...
 * appears. In the resulting window, click on the <i>Arguments</i> tab, and
 * type the command-line arguments in the text box labeled "Program Arguments".
 * Then, click <i>Apply</i> and run the program.
 * <p>
